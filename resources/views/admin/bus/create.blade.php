...
<div class="container mt-4">
    ...
    <form action="{{ route('admin.buses.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" id="bus_number" name="bus_number" class="form-control" value="{{ old('bus_number') }}" required>
        </div>

        <!-- Bus Name / Model -->
        <div class="form-group mb-3">
            <label for="bus_name">Bus Name / Model</label>
            <input type="text" id="bus_name" name="bus_name" class="form-control" value="{{ old('bus_name') }}" required>
        </div>
        ...
        <div class="form-group mb-3">
            ...
            <select id="bus_type" name="bus_type" class="form-select" required>
                ...
                <option value="Regular" {{ old('bus_type') == 'Regular' ? 'selected' : '' }}>Regular</option>
                <option value="AC" {{ old('bus_type') == 'AC' ? 'selected' : '' }}>AC</option>
                <option value="Deluxe" {{ old('bus_type') == 'Deluxe' ? 'selected' : '' }}>Deluxe</option>
                <option value="Sleeper" {{ old('bus_type') == 'Sleeper' ? 'selected' : '' }}>Sleeper</option>
            </select>
        </div>
        ...
        <div class="form-group mb-3">
            ...
            <input type="number" id="total_seats" name="total_seats" class="form-control" value="{{ old('total_seats') }}" min="1" required>
        </div>
